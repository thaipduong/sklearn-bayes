...
upper_grid, upper_grid2, upper_grid3, upper_grid4, upper_line5 = rvm.predict_upperbound(Xgrid)
...
#            linestyles="solid")
cs3 = plt.contour(X1, X2, np.reshape(upper_grid4, (n_grid, n_grid)), levels=[0.0], cmap="Greys_r",
                linestyles="dashdot", label="$G_3(x)$", linewidths=6)
cs1 = plt.contour(X1, X2, np.reshape(rv_grid, (n_grid, n_grid)), levels=[0.4945], cmap="Greys_r", label="$G_1(x)$", linewidths=6)
#radius = rvm.get_radius(line_seg, A)
#print("radius", radius)
#ax = plt.axes()
#circle = plt.Circle(A, radius, color='xkcd:orange', fill=False, label="free ball")
...
for i in range(T):
    Bi = A + v*np.array([np.cos(2*i*np.pi/T), np.sin(2*i*np.pi/T)])
    intersect = rvm.predict_upperbound_line_check(line_seg, A, Bi)
...
#h2, _ = cs2.legend_elements()
h3, _ = cs3.legend_elements()
#plt.legend([h1[0], h3[0]], \
#           ['$G_1(x) = 0$','$G_3(x) = 0$'], loc = 2, fontsize=15,framealpha=0.5)
plt.savefig("/home/erl/repos/sklearn-bayes/figs/rvm_model_checking_lines_video.pdf", bbox_inches='tight', pad_inches=0)
...
cs3 = plt.contour(X1, X2, np.reshape(upper_grid4, (n_grid, n_grid)), levels=[0.0], cmap="Greys_r",
                linestyles="dashdot", label="$G_3(x)$")
cs1 = plt.contour(X1, X2, np.reshape(rv_grid, (n_grid, n_grid)), levels=[0.4945], cmap="Greys_r", label="$G_1(x)$")
#radius = rvm.get_radius(line_seg, A)
#print("radius", radius)
#ax = plt.axes()
#circle = plt.Circle(A, radius, color='xkcd:orange', fill=False, label="free ball")
...
while radius > 0.1 and curr_t < max_t:
    ...
    radius = rvm.get_radius(line_seg, [curr_x, curr_y])
    print("radius = ", radius)
    curr_t = next_t
...
#h2, _ = cs2.legend_elements()
h3, _ = cs3.legend_elements()
plt.legend([h1[0], h3[0], curve_colliding[0], curve_free[0]], \
           ['$G_1(x) = 0$', '$G_3(x) = 0$', 'colliding curve', "free curve"], loc = 1, fontsize=15,framealpha=0.5)
plt.savefig("/home/erl/repos/sklearn-bayes/figs/rvm_model_checking_curves_video.pdf", bbox_inches='tight', pad_inches=0)
...
h3, _ = cs3.legend_elements()
if colliding_curve:
    plt.legend([h1[0], h3[0], curve_colliding[0]], \
               ['$G_1(x) = 0$', '$G_3(x) = 0$', 'colliding curve'], loc = 2, fontsize=15,framealpha=0.5)
    plt.savefig("/home/erl/repos/sklearn-bayes/figs/rvm_model_checking_curves_colliding_video.pdf", bbox_inches='tight', pad_inches=0)
else:
    plt.legend([h1[0],  h3[0], curve_free[0]], \
               ['$G_1(x) = 0$','$G_3(x) = 0$', "free curve"], loc=2, fontsize=15, framealpha=0.5)
    plt.savefig("/home/erl/repos/sklearn-bayes/figs/rvm_model_checking_curves_free_video.pdf", bbox_inches='tight',
                pad_inches=0)
