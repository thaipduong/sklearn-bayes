...
def update_precisions(Q,S,q,s,A,active,tol,n_samples,clf_bias, skip = 0):
    ...
    if count_k_nearest:
        ...
        if skip > 0:
            rv_feature_index = np.argmax(deltaL[1:skip])
    ...
    #for i in range(skip):
    #    if theta[i] > 0:
    #        # if active[feature_index] == True:
    #        #    print("alpha before: ", A[feature_index], "alpha after: ", s[feature_index]**2 / theta[feature_index])
...
class ClassificationARD5(BaseEstimator,LinearClassifierMixin):
    '''
    ...
    def __init__(self, n_iter=50, tol=1e-1, n_iter_solver=15, normalize=False,
                 tol_solver=1e-2, A_thres = 3.0, fit_intercept=INTERCEPT, fixed_intercept =UNKNOWN_PROB, verbose=False):
        self.n_iter             = n_iter
        self.tol                = tol
        print("Init ", self.tol)
        ...
        #print("3self.A_thres = ", self.A_thres)
    ...
    def fit(self,X,y):
        ...
        #print("2self.A_thres = ", self.A_thres)
    ...
    def _fit(self,X,y):
        ...
        if self.prev_trained: # i.e. there is an existing model trained previously.
            ...
            active[0] = True
            A[0] = 0.001  # np.finfo(np.float16).eps
        ...
        for j in range(len(A)):
            if j >= skip and A[j] > self.A_thres:
                #print(A[j], self.A_thres)
                A[j] = np.PINF
                active[j] = False
        #print(A)
        #print(active)
    ...
    def _posterior_dist(self, X, y, A, keep_prev_mean = True, tol_mul = 1.0, global_laplace = False):
        '''
    ...
    def _posterior_dist_final(self, X, y, A, keep_prev_mean = True, tol_mul = 1.0, global_laplace = False):
        '''
        Uses Laplace approximation for calculating posterior distribution
        '''
        f_full = lambda w: _gaussian_cost_grad(X, y, w, A)
        ...
        #print("compare min eigs", np.min(sparse_eigs.real), np.min(eigs.real))
...
class RVC5(ClassificationARD5):
    ...
    [2] Analysis of sparse Bayesian learning (Tipping & Faul 2001)
        (http://www.miketipping.com/abstracts.htm#Faul:NIPS01)
    '''
    def __init__(self, n_iter = 200, tol = 1e-1, n_iter_solver = 50, tol_solver = 1e-3, A_thres = 3.0,
        ...
        super(RVC5,self).__init__(n_iter,tol,n_iter_solver,False,tol_solver, A_thres,
        ...
        self.prev_y = None
        self.total_time = []
        self.kernelcal_fit_time = []
        self.fit_time = []
        self.full_posterior_time = []
        self.lambda_max_time = []
        
        
    def fit(self,X,y, pos_neg_rescale = False, r_mul = 1.0, k_nearest = 50, use_diff = True):
        '''
        ...
        if self.prev_trained:
            ...
            self.prev_A = local_A
            if pos_neg_rescale and not use_diff:
        ...
        #print("############################################### rv_global length before update = ",
        #      len(self.relevant_vectors_dict.keys()))
        count_dict = {}
        ...
        print("############################################### rv_global length after = ", len(self.relevant_vectors_dict.keys()))
        ...
        stime_ext = time.time()
        self.recover_posterior_dist(tol_factor = 0.1)
        time4 = time.time()
        print("@@@@@@@@@@@@@@@@@@@@@@@@ EXTRA TIME:", time4 - stime_ext)
        self.prev_X = self.cur_X
        self.prev_y = self.cur_y
        self.total_time.append(time4 - time0)
        self.kernelcal_fit_time.append(time3 - time1)
        self.fit_time.append(time3 - time2)
        print("Update time:", time4 - time0, time3 - time1, time3 - time2)
        return self

    def recover_posterior_dist(self, tol_factor = 1.0, a_thres = -1.0):
        self.all_rv_X = []
        self.all_rv_y = []
        self.all_rv_A = []
        ...
        print("min of all_rv_A", np.min(all_rv_A))
        full_posterior_start = time.time()
        Mn, Sn, cholesky = self._posterior_dist_final(all_rv_K, all_rv_y, all_rv_A, keep_prev_mean=False,
                                                          tol_mul=tol_factor, global_laplace=True)
        ...
        self.Mn = Mn
        self.Sn = Sn
        full_posterior_end = time.time()
        self.full_posterior_time.append(full_posterior_end - full_posterior_start)
        print("Full posterior time: ", full_posterior_end - full_posterior_start)
        approx_posterior_start = time.time()
        self.Mn_approx, self.lambda_max= self._posterior_dist_final_approx(all_rv_K, all_rv_y, all_rv_A, keep_prev_mean=False,
                                                      tol_mul=tol_factor, global_laplace=True)
        approx_posterior_end = time.time()
        self.lambda_max_time.append(approx_posterior_end - approx_posterior_start)
        print("Approx posteror lambda max time: ", approx_posterior_end - approx_posterior_start)
    ...
    def predict_proba(self, X):
        ...
        probs: numpy array of size (n_samples_test,)
           Estimated probabilities of target classes
        '''
        decision, var, K = self.decision_function(X)
    ...
    def predict_proba_approx(self, X):
        ...
        prob[decision < 0] = norm.cdf(decision[decision < 0]/np.sqrt(self.lambda_max*np.sum(K[decision < 0,:]**2, axis=1) + 1))
        if prob.ndim == 1:
            prob = np.vstack([1 - prob, prob]).T
        prob = prob / np.reshape(np.sum(prob, axis=1), (prob.shape[0], 1))
        return prob, var, decision,  np.sqrt(var + 1)
